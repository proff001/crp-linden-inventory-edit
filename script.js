...
let kg = Intl.NumberFormat('nb-NO', {
	style: 'unit',
...
let gram = Intl.NumberFormat('nb-NO', {
	style: 'unit',
...
let money = Intl.NumberFormat('nb-NO', {
	style: 'currency',
	currency: 'NOK',
	minimumFractionDigits: 0
});
...
Display = function(bool) {
	if (bool) {
		...
		$.when($(".background").fadeOut(200)).done(function() {
			$(".iteminfo").hide();
		});
	}
}
...
HSN.InventoryGetDurability = function(quality) {
	...
	let color = colorMixer([46, 204, 113], [231, 76, 60], (quality/100));
	let width = quality+"%"
...
$(document).on("mousedown", ".ItemBoxes", function(e){
	if(e.which == 2) {
		if ($(this).data("location") === undefined) {
			let fromData = $(this).data("ItemData");
        	if ((fromData !== undefined)) {
            	e.preventDefault();
				inv = $(this).parent().data('invTier')
				$.post("https://linden_inventory/useItem", JSON.stringify({
                	item: fromData,
                	inv: inv
            	}));
